...
public class DAOBooksImpl extends Database implements DAOBooks {
    ...
    public void registrar(Books book) throws Exception {
        try {
            this.Conectar();
            try (java.sql.PreparedStatement st = this.conexion.prepareStatement("INSERT INTO books(title, date, author, category, edit, lang, pages, description, ejemplares, stock, available) VALUES(?,?,?,?,?,?,?,?,?,?,?)")) {
    ...
    public void modificar(Books book) throws Exception {
        try {
            this.Conectar();
            try (java.sql.PreparedStatement st = this.conexion.prepareStatement("UPDATE books SET title = ?, date = ?, author = ?, category = ?, edit = ?, lang = ?, pages = ?, description = ?, ejemplares = ?, stock = ?, available = ? WHERE id = ?")) {
                st.setString(1, book.getTitle());
                st.setString(2, book.getDate());
                st.setString(3, book.getAuthor());
                st.setString(4, book.getCategory());
    ...
    public void eliminar(int bookId) throws Exception {
        try {
            this.Conectar();
            try (java.sql.PreparedStatement st = this.conexion.prepareStatement("DELETE FROM books WHERE id = ?")) {
                st.setInt(1, bookId);
                st.executeUpdate();
            }
        } finally {
    ...
    public Books getBookById(int bookId) throws Exception {
        ...
        try {
            this.Conectar();
            try (java.sql.PreparedStatement st = this.conexion.prepareStatement("SELECT * FROM books WHERE id = ? LIMIT 1")) {
                st.setInt(1, bookId);
                try (java.sql.ResultSet rs = st.executeQuery()) {
                    while (rs.next()) {
                        book = new Books();
